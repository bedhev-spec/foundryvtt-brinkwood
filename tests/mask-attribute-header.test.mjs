...
test("Mask Attributes live in the identity header and share Character presentation", async () => {
  ...
  assert.match(maskStyles, /\.mask-sheet__identity-block\s*\{[\s\S]*?grid-template-columns:\s*minmax\(150px, 200px\) minmax\(230px, 1fr\) minmax\(260px, 0\.9fr\);[\s\S]*?column-gap:\s*20px;/);
