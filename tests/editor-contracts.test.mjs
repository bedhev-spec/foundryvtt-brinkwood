...
test("v13 image actions and editors are editable-only", async () => {
  ...
  for (const template of templates.slice(1, 7)) assert.match(template, /\{\{#if editable\}\}\s*data-action="editImage" data-edit="img"/); assert.match(templates[0], /parts\/sheet-identity-portrait\.html/); assert.match(templates[0], /parts\/sheet-notes\.html/); assert.match(templates[7], /\{\{#if editable\}\}[\s\S]*?<prose-mirror class="sheet-notes__editor" name="\{\{fieldName\}\}"[^>]*data-document-uuid="\{\{documentUuid\}\}"[^>]*collaborate toggled>/);
...
test("native rich-text controls submit source fields while viewers receive enriched content", async () => {
  ...
  assert.match(notesPartial, /name="\{\{fieldName\}\}" value="\{\{fieldValue\}\}" data-document-uuid="\{\{documentUuid\}\}" collaborate toggled>\{\{\{enrichedContent\}\}\}<\/prose-mirror>/);
  assert.match(notesPartial, /\{\{else\}\}\s*<div class="editor editor-content sheet-notes__preview">\{\{\{enrichedContent\}\}\}<\/div>/);
