...
test("Character Effects retain accessible sub-tabs and shared v13 state", async () => {
  const [effects, actorTemplate, controller, sharedController, categories, styles, stylesheet, manifest] = await Promise.all([
    read("templates/parts/active-effects.html"), read("templates/actor-sheet.html"), read("module/blades-actor-sheet.js"),
    read("module/blades-sheet.js"), read("module/blades-active-effect.js"), read("scss/import/sheet-tabs.scss"),
  ...
  assert.match(actorTemplate, /id="character-\{\{_id\}\}-effects"[\s\S]*?role="tabpanel"[\s\S]*?active-effects\.html/);
  ...
  assert.match(styles, /&\.actor\.pc\.character \.effects-tabs/);
  assert.doesNotMatch(stylesheet, /import\/legacy-character-effects\.scss/);
